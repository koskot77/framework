...
void AppAgentWrapper::addParameter(AppParameter_t *par){
...
void AppAgentWrapper::removeParameter(AppParameter_t *par){
        int res = hashOfParameters.erase(par->name());
        if( !res ) cerr<<"Parameter '"<<par->name()<<"' not found"<<endl;
}

void AppAgentWrapper::setParameter(const char* parname, const char* value){
...
string AppAgentWrapper::getParameter(const char* parname) const {
