...
edm::Wrapper<reco::PFJetCollection>             *__pfJets = new edm::Wrapper<reco::PFJetCollection>();
...
//    inputJetsBranch->SetAddress(&__patJets);
    ...
    TBranch *inputJetsBranch = Events->GetBranch("recoPFJets_ak5PFJets__HLT.");
    if( !inputJetsBranch )
        return AppResult(AppResult::STOP|AppResult::ERROR,"No 'recoPFJets_ak5PFJets__HLT.' branch found");
    inputJetsBranch->SetAddress(&__pfJets);
...
///      for(vector<pat::Jet>::const_iterator pjet = __patJets->product()->begin(); pjet != __patJets->product()->end(); pjet++){
    if( __pfJets->isPresent() )
      for(vector<reco::PFJet>::const_iterator pjet = __pfJets->product()->begin(); pjet != __pfJets->product()->end(); pjet++){
