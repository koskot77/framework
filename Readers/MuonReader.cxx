...
edm::Wrapper<reco::MuonCollection> *__pfMuons = new edm::Wrapper<reco::MuonCollection>();
...
*/
    TBranch *inputMuonsBranch = Events->GetBranch("recoMuons_muons__HLT.");
    if( !inputMuonsBranch )
        return AppResult(AppResult::STOP|AppResult::ERROR,"No 'recoMuons_muons__HLT.' branch found");
    inputMuonsBranch->SetAddress(&__pfMuons);
...
//      for(vector<pat::Muon>::const_iterator pmuon = __patMuons->product()->begin(); pmuon != __patMuons->product()->end(); pmuon++){
    if( __pfMuons->isPresent() )
      for(vector<reco::Muon>::const_iterator pmuon = __pfMuons->product()->begin(); pmuon != __pfMuons->product()->end(); pmuon++){
