...
AppResult::SKIP   | move on the the next event (= continue) 
...
objects](https://github.com/koskot77/framework/blob/master/workdir/tutorial/MyModule3.h#L19-L20) (note the `const` qualifier!)
...
[MyModule5](https://github.com/koskot77/framework/blob/master/workdir/tutorial/MyModule5.h) shows how to build a mathematical expression:
...
In this example the [CompiledFormula](https://github.com/koskot77/framework/blob/master/AnUtils/CompiledFormula.h) container is populated
...
and be able to call `beginRun` and `endRun` methods of all of the modules using `beginRunNotify` and `endRunNotify` calls
...
but requires serializer/deserializer functions to be provided for every storable data
...
It was also worked with the [CMS](http://cms.cern.ch) detector using the following data processing code:
